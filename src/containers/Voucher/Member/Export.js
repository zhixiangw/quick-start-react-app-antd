...
class ExportMVourcherForm extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      loading: false,
  ...
  render() {
    ...
    const sopts = memberVoucher.map(item => (<Select.Option key={item.id} value={item.id}>{item.name}</Select.Option>));
    return (
      <Form onSubmit={this.handleSubmit} className="login-form">
        <Form.Item {...formItemLayout} label="兑换会员卡">
          {getFieldDecorator('memberVourId', {
            rules: [{
              required: true,
              message: '请选择会员卡',
              type: 'integer'
...
const mapDispatchToProps = dispatch => ({
  queryAllMVoucher: payload => dispatch(Action.allVmember(payload)),
  onSubmit: payload => dispatch(Action.exportVmember(payload)),
});

const WrappedNormalLoginForm = Form.create({ name: 'vexportMVoucher_form' })(connect(mapStateToProps, mapDispatchToProps)(ExportMVourcherForm));
